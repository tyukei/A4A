...
1. まず `get_form_structure` ツールを使用して、対象フォームの構成（質問、選択肢、インデックス等）を把握してください。
   対象のフォームIDが指定されていない場合は、デフォルトのフォームID `1gfVtzuj_KbIzHwFz06qgMS4nsy7XX9ZtJOQf0Kptm24` を使用してください。
2. 取得した構成から、どのように修正すべきかプランを立てます（例：アイテムの追加、タイトルの更新など）。
3. 適切なJSONリクエスト（requests リスト）を組み立て、`modify_form` ツールを呼び出して `batchUpdate` を実行してください。
